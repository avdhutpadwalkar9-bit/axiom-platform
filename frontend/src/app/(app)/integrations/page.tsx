  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 max-w-[1400px]">
        ...
        <div className="flex items-center gap-1 bg-white rounded-lg border border-[#e5e5e5] p-1">
          <button
            ...
            className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${
              filter === "all" ? "bg-[#f5f5f5] text-[#1a1a1a]" : "text-[#999] hover:text-[#666]"
            }`}
          >
            All
          </button>
          {categories.map((cat) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${
                filter === cat ? "bg-[#f5f5f5] text-[#1a1a1a]" : "text-[#999] hover:text-[#666]"
              }`}
            >
              {cat}
            </button>
        ...
        {filtered.map((integration, i) => (
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
                <div
                  className="w-12 h-12 rounded-xl flex items-center justify-center text-[#1a1a1a] font-bold text-sm"
            ...
            {integration.status === "connected" ? (
              <div className="flex items-center justify-between">
                ...
                <button className="text-xs text-[#999] hover:text-[#1a1a1a] transition-colors">
                  Settings
                </button>
              </div>
            ) : (
              <button className="w-full py-2 rounded-lg bg-[#fafafa] border border-[#e5e5e5] text-xs text-[#666] hover:bg-[#f5f5f5] transition-all flex items-center justify-center gap-1.5">
      ...
      <div className="bg-white rounded-xl border border-[#e5e5e5] p-6">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-sm font-semibold text-[#1a1a1a] mb-1">Custom Integration</h3>
            <p className="text-xs text-[#999]">
              Use our REST API to connect any data source. Full documentation available.
            </p>
          </div>
          <button className="flex items-center gap-1.5 px-4 py-2 bg-[#fafafa] border border-[#e5e5e5] rounded-lg text-xs text-[#666] hover:bg-[#f5f5f5] transition-colors">
